...
Using the Box-Cox transformation, the BcTransform command gave me the value of lambda to be around 0.1414. Then I named the boxcox transformation data as training.bc. I do notice that 0 is in the confidence interval, so a log transformation could be needed.
...
Then, I want to check if training.log needed any differencing to remove trend. To start, I differenced the log of training set, then at d=2, and d=3. All differencing took place at lag 1.
...
peak at 9, no at 18, q= 0:1 because 
## Model Estimation
...
Based from the ACF of dat.1, lags 1, 9, 10,11 were outside the confidence interval and for PACF lags 1 and 9 were outside. Therefore, I suspected there might be some seasonality and the potential use of the SARIMA model, with period at 9. 

In both the ACF and PACF, the peak is at lag 9 while lag 18 does not show to be outside. Thus in my for loop,  I put values of both P and Q to be 0 or 1. As a result, p and q will also take on the same values as P and Q because both ACF and PACF showed lag 1 to be outside.
...
Based from the lowest AICc, I will consider the two best models, which are SARIMA (1,1,0),(1,0,0) , seasonality at 9 and SARIMA (1,1,0), (1,0,1), seasonality at 9. 


Model 1 will be at (1,1,0),(1,0,0) , seasonality 9 
...
I determined that model 1 is stationary because its phi value is less than the absolute value of 1 and all AR models are invertible.

Second, I test fit2 at (1,1,0),(1,0,1), period=9.
...
The ACF and PACF of residuals shows no lags outside of confidence interval. The fitted residuals are to AR(0), so it is white noise. This passes diagnostic checking for model 2.

## Best model

Since the analysis of residuals in model 1 and 2 was satisfactory and pass diagnostic checking, I will consider principle of parsimony, therefore I will only use fit1 due to less parameters.

```{r}
fit1
```

Final model should be $(1-0.83B)(1+0.566B^9)(1-B)X_t = Z_t, Z_t ~ WN(0, 0.00271)$
