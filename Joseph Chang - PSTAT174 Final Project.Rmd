...
At dat.2, I noticed that lag 9 was the furthest-most lag outside of confidence interval for ACF. Lag 1 is also outside. The same could be said for PACF, with lags 1 and 9 outside confidence interval

```{r}
hist(training, col = "light blue", xlab = "", main = "Histogram of data")
...
```{r}
# Final model 1
fit2 <- arima(training, order = c(0,2,1), method = "ML")
fit2
AICc(fit2)
...
# Final model 2
fit2 <- arima(training, order = c(1,1,1), method = "ML")
fit2
AICc(fit2)
# stationary because less than abs 1, and all AR are invertible
...
ts.plot(c(lambda*training +1)^(1/lambda), xlim=c(1, length(lambda*training)+10), ylim=c(100,1200) , xlab = "Years after 1960", ylab = "Billions of USD")
