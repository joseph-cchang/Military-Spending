...
# Final model 2
new_fit2 <- arima(training.log, order = c(0,1,8), fixed = c(0,0,0,0,0,0,0,0), method = "ML")
new_fit2
AICc(new_fit2)
...
fit3 <- arima(training.log, order = c(10,1,0), method = "ML")
fit3
AICc(fit3)

new_fit3 <- arima(training.log, order = c(10,1,0),fixed=c(NA,0,0,0,0,0,0,0,NA,NA), method = "ML")
new_fit3
AICc(new_fit3)
...
ts.plot(training.log, xlim=c(1, length(training.log)+15), ylim=c(min(training.log), max(mypred$pred +2*mypred$se)) , xlab = "Years after 1960", ylab = "Billions of USD")
