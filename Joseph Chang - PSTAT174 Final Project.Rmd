...
new_fit1 <- arima(spent.train, order = c(1,1,1), 
             method = "ML")
new_fit1
# From fit1, it seems that I may have overdifferenced so I will use d=1 in diagnotics
...
# AR (9)
first_fit <- arima(spent.train, order=c(9,1,0), fixed=c(0,0,0,0,0,0,0,0, NA))

# MA (9)
second_fit <- arima(spent.train, order=c(0,1,9), fixed=c(0,0,0,0,0,0,0,0,NA))

# ARMA(9,9)
third_fit <- arima(spent.train, order=c(9,1,9), fixed=c(0,0,0,0,0,0,0,0,NA, 0,0,0,0,0,0,0,0,NA))

# From the three models, ARMA(9,9) produced the smallest AICc
...
# AR (9)
fit_1 <- arima(spent.train, order=c(9,2,0), fixed=c(0,0,0,0,0,0,0,0, NA))

# MA (9)
fit_2 <- arima(spent.train, order=c(0,2,9), fixed=c(0,0,0,0,0,0,0,0,NA))

# ARMA(9,9)
fit_3 <- arima(spent.train, order=c(9,2,9), fixed=c(0,0,0,0,0,0,0,0,NA, 0,0,0,0,0,0,0,0,NA))
