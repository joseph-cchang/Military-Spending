...
Then, I want to check if spent needs any differencing. To start, I plotted the time series for the training set for spent differenced once, then twice, then three times. As I increased difference each time, I plotted its time series for comparison. diff at lag 1
...
for (i in 1:nrow(df)) {
  sarima.obj <- NULL
  try(arima.obj <- arima(spent.train, order=c(df$p[i],1, df$q[i]), method="ML"))
...
new_fit1 <- arima(spent.train, order = c(1,1,0), seasonal = list (order = c(1,0,0), period = 9), method = "ML")
new_fit1
AICc(new_fit1)
# rerun the for loop
...
# Final model 2
fit2 <- arima(spent.train, order = c(1,1,1), method = "ML")
fit2
AICc(fit2)
# stationary because less than abs 1, and all AR are invertible

# see uc.check *unit circle
# source("plot.roots.R")
